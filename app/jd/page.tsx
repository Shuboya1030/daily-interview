      ...
      {(streamedText || analyzing) && (
        <div className="mb-8">
          ...
          <div className="bg-cream-dark/30 border border-cream-dark rounded-lg p-6">
            <div className="prose prose-sm max-w-none text-ink/85 leading-relaxed jd-predictions">
              {streamedText.split('\n').map((line, i) => {
                if (line.startsWith('### ')) {
                  return (
                    <h3 key={i} className="text-base font-semibold text-ink mt-4 mb-1 first:mt-0">
                      {line.replace('### ', '')}
                    </h3>
                  )
                }
                if (line.startsWith('**Why:**')) {
                  return (
                    <p key={i} className="text-sm text-ink/60 mb-0.5">
                      {line.replace('**Why:**', '').replace(/\*\*/g, '').trim()}
                    </p>
                  )
                }
                if (line.startsWith('**Level:**')) {
                  const level = line.replace('**Level:**', '').replace(/\*\*/g, '').trim()
                  const color =
                    level.includes('Advanced') ? 'bg-red-100 text-red-700' :
                    level.includes('Intermediate') ? 'bg-yellow-100 text-yellow-700' :
                    'bg-green-100 text-green-700'
                  return (
                    <span key={i} className={`inline-block px-2 py-0.5 text-xs rounded mb-3 ${color}`}>
                      {level}
                    </span>
                  )
                }
                if (line.trim()) {
                  return <p key={i} className="text-sm text-ink/75">{line}</p>
                }
                return null
              })}
              {analyzing && (
            ...
            {done && (
              <div className="mt-4 pt-3 border-t border-cream-dark text-xs text-ink/40">
